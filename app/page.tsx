...
import { ArrowRight, BookOpen, MapPinned, Stethoscope } from "lucide-react";
...
export default async function AppPickerPage() {
  ...
  return (
    <main className="min-h-dvh flex-1 bg-[radial-gradient(circle_at_top_left,rgba(244,114,182,0.24),transparent_35%),radial-gradient(circle_at_bottom_right,rgba(14,165,233,0.18),transparent_42%),linear-gradient(135deg,#fff1f7_0%,#ffffff_44%,#eaf6ff_100%)] px-4 py-5 text-slate-950 dark:bg-none dark:bg-background dark:text-foreground sm:px-6 sm:py-8">
      <div className="mx-auto flex min-h-[calc(100dvh-4rem)] w-full max-w-6xl flex-col">
        <header className="flex items-center justify-between gap-3">
          ...
          >
            <Image
              ...
              width={1024}
              height={1536}
              className="size-11 object-cover object-top drop-shadow-sm"
              priority
            />
            <span>Mausi App</span>
          </Link>
        ...
        <section className="grid flex-1 items-center gap-8 py-8 sm:py-10 lg:grid-cols-[0.88fr_1.12fr]">
          <div className="max-w-xl">
            <p className="text-sm font-medium uppercase tracking-[0.18em] text-rose-600 dark:text-rose-300">
              Für Mausi
            </p>
  ...
  return (
    <Card className="group overflow-hidden rounded-lg bg-white/90 shadow-[0_18px_55px_rgba(15,23,42,0.12)] ring-slate-200/80 transition duration-300 hover:-translate-y-1 hover:ring-slate-400/50 dark:bg-card dark:ring-border">
      ...
      <CardContent>
        <Link
          href={href}
          className="relative grid aspect-[4/3] place-items-center overflow-hidden rounded-lg bg-[linear-gradient(135deg,rgba(147,197,253,0.22),rgba(251,113,133,0.18),rgba(244,114,182,0.14))] ring-1 ring-black/5 dark:ring-white/10"
          aria-label={title}
        >
          <Image
            src={imageSrc}
